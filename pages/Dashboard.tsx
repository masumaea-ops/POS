...
const Dashboard: React.FC = () => {
  ...
  const triggerDunSMS = (custId: number, name: string, phone: string, outstanding: number, aged60: number, aged90: number) => {
    ...
    // Parse customized dunning SMS body
    ...
    <div className="p-4 md:p-8 bg-slate-50 dark:bg-slate-900 min-h-full pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5 mb-8">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {settings.corpName} • Distributor Support & POS Command
          </p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center gap-3 text-xs">
          <div className="flex items-center gap-1.5">
             ...
             <span className="font-bold text-slate-700 dark:text-slate-300 font-mono">COMPLIANCE HUB ACTIVE</span>
          </div>
          <span className="text-slate-300 dark:text-slate-600">|</span>
          <span className="font-semibold text-slate-500 dark:text-slate-400">Node: {settings.branchCode}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-10">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 lg:col-span-8 flex flex-col justify-between">
           <div>
             <div className="flex justify-between items-start">
               <div>
                 <h3 className="text-lg font-black text-slate-950 dark:text-white animate-fade-in">Hourly Sales Target Volatility</h3>
             ...
             <div className="relative mt-8 h-48 bg-slate-50/50 dark:bg-slate-900/30 rounded-xl border border-slate-100 dark:border-slate-800 p-4 flex items-center justify-center">
                 ...
                 {hoveredHour && (
                   <div 
                     className="absolute bg-slate-900 border border-slate-755 text-white p-2 text-[10px] rounded shadow-2xl pointer-events-none transition-all duration-150 z-20"
           ...
           <div className="border-t border-slate-100 dark:border-slate-850 pt-4 mt-4 flex items-center justify-between text-xs text-slate-500">
              <span>Peak Distribution Traffic: <strong className="text-slate-800 dark:text-slate-200">12:00 - 16:30</strong></span>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 lg:col-span-4 flex flex-col justify-between">
                ...
                {gateways.map(g => (
                  <div 
                     ...
                     className={`p-3.5 rounded-xl border transition-all ${
                       hoveredGate === g.key 
                       ? 'bg-slate-50 dark:bg-slate-750/70 border-slate-200 dark:border-slate-705 scale-[1.02]' 
           ...
           <div className="border-t border-slate-100 dark:border-slate-850 pt-4 mt-4 text-[10px] text-slate-400 font-mono text-center uppercase tracking-wide">
              {settings.currency === 'KES' ? 'M-Pesa' : 'Mobile Gateway'} Dominates: 55% Share
           </div>
        </div>
      </div>
      ...
      <div className="mt-10 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-755 pb-4 mb-4">
             ...
             {/* Filter Tabs */}
             <div className="flex p-1 bg-slate-100 dark:bg-slate-900 text-xs font-bold rounded-lg border dark:border-slate-700 shrink-0 select-none">
                 <button 
                   onClick={() => setActiveSegment('all')}
                   className={`px-3.5 py-1.5 rounded ${activeSegment === 'all' ? 'bg-white dark:bg-slate-800 shadow-xs text-brand-orange' : 'text-slate-600 dark:text-slate-355'}`}
                 >
                   All Accounts
                 </button>
                 <button 
                   onClick={() => setActiveSegment('risk')}
                   className={`px-3.5 py-1.5 rounded ${activeSegment === 'risk' ? 'bg-white dark:bg-slate-800 shadow-xs text-amber-600' : 'text-slate-600 dark:text-slate-355'}`}
                 >
             ...
             <table className="w-full text-left text-xs">
                 <thead className="bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold uppercase tracking-wide">
                     <tr>
                         ...
                         <th className="p-3 text-right font-medium text-slate-500">0-30 Days Aging</th>
                         <th className="p-3 text-right font-medium text-slate-500">31-60 Days Aging</th>
                     ...
                     {agedAccounts
                       ...
                       .map(client => (
                          <tr key={client.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-750/30 font-mono">
                              ...
                              <td className="p-4">{formatPrice(client.limit)}</td>
                              <td className="p-4 text-right font-black">{formatPrice(client.outstanding)}</td>
                              <td className="p-4 text-right">{formatPrice(client.aged30)}</td>
                              <td className="p-4 text-right">{formatPrice(client.aged60)}</td>
                              <td className="p-4 text-right text-rose-500 font-extrabold">{formatPrice(client.aged90)}</td>
                              <td className="p-4 text-center">
                                  {dunStatus[client.id] === 'sent' ? (
                                     ...
                                     <button 
                                       ...
                                       className="py-1 px-2.5 bg-brand-orange hover:bg-brand-orange/90 text-white rounded font-bold text-[10px] uppercase shadow-xs hover:scale-105 transition-all focus:outline-none"
                                     >
         ...
         <div className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs flex justify-between font-bold text-slate-600 dark:text-slate-400">
            <span>Aged Receivables Pool Value: <strong className="text-slate-900 dark:text-white font-mono">{formatPrice(totalOutstandingAR)}</strong></span>
